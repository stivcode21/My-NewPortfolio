...
export default function Carousel({
	...
	const handleDragMove = (e) => {
		...
		carouselRef.current.style.transform = `translateX(calc(-${currentImageIndex * 100
			}% + ${distance}px))`;
	};
						...
						<>
							{showControls && (
								<div className={styles.carouselControl}>
									<HoverLabel position="top" label={"Anterior"} >
										<button
									...
									<HoverLabel position="top" label={"Siguiente"} >
										<button
								...
								<ol className={styles.stepbar}>
									{carouselImages.map((_, index) => (
										<li
											...
											className={`${styles.step} ${index === currentImageIndex ? styles.active : ""
												}`}></li>
									))}
								</ol>
							)}
